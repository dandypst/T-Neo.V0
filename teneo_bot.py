    ...
    async def send_request(self, command):
        ...
        while time.time() < deadline:
            try:
                ...
                if t in ("ping", "pong", "heartbeat"):
                    continue

                # Log semua pesan non-sistem untuk debug
                log_res(f"[MSG] type={t!r} keys={list(data.keys())} content={str(data.get('content',''))[:80]}")
                ...
                # type=message langsung dari agent = sukses tanpa payment
                if t == "message" and data.get("from") != "system":
