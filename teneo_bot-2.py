...
class TeneoBot:
    ...
    async def send_request(self, command):
        ...
        payload = {
            ...
            "content":    command,
            "data":       {"command": command, "content": command},
            "timestamp":  now_iso(),
        }
        ...
        while time.time() < deadline:
            try:
                ...
                if t in ("task_result", "result", "response", "task_complete",
                         "task_response", "output", "reply"):
                    val = data.get("content") or data.get("data") or data.get("result") or "ok"
                    return True, str(val)[:100]

                # type=message = response dari agent (server pakai format sama)
                ...
                # Error
                if t == "error":
                    val = data.get("data") or data.get("message") or data.get("content") or "error"
